...
class enrol_self_plugin extends enrol_plugin {
    ...
    public function get_instance_name($instance) {
        ...
        if (empty($instance->name)) {
            if (!empty($instance->roleid) and $role = $DB->get_record('role', array('id'=>$instance->roleid))) {
                $role = ' (' . role_get_name($role, context_course::instance($instance->courseid, MUST_EXIST)) . ')';
            } else {
                $role = '';
            }
            $enrol = $this->get_name();
    ...
    public function get_action_icons(stdClass $instance) {
        ...
        $context = context_course::instance($instance->courseid, MUST_EXIST);//WM
        //$context = get_context_instance(CONTEXT_COURSE, $instance->courseid);
    ...
    public function get_newinstance_link($courseid) {
        $context = context_course::instance($courseid, MUST_EXIST);//WM
    ...
    protected function email_welcome_message($instance, $user) {
        ...
        $context = context_course::instance($course->id, MUST_EXIST);//ADD BY WM 220217    
        //$context = get_context_instance(CONTEXT_COURSE, $course->id);//REM BY WM 220217
        $rusers = array();
        if (!empty($CFG->coursecontact)) {
            $croles = explode(',', $CFG->coursecontact);
            //$rusers = get_role_users($croles, $context, true, '', 'r.sortorder ASC, u.lastname ASC');//REM BY WM 220217
        }
        if ($rusers) {
