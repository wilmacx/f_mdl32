...
class enrol_self_edit_form extends moodleform {
    ...
    function definition() {
        ...
        $mform->setType('name', PARAM_TEXT);//ADD BY WM 220217
        ...
        $sql = "select sc.fullname,ca.* FROM {$CFG->prefix}course_availability ca join {$CFG->prefix}course sc on (sc.id = ca.sourcecourseid) WHERE courseid='{$context->instanceid}'";
