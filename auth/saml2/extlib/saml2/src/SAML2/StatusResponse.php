...
abstract class SAML2_StatusResponse extends SAML2_Message
{
	...
	if ($statusCode->getAttribute('Value') =='urn:oasis:names:tc:SAML:2.0:status:Responder') {
                $this->status['Code'] = 'User not authorized. Please contact the helpdesk';
        } else {
         	$this->status['Code'] = $statusCode->getAttribute('Value');
        }
        ...
        if (!empty($subCode)) {
		if ($subCode[0]->getAttribute('Value') =='urn:oasis:names:tc:SAML:2.0:status:RequestDenied') {
