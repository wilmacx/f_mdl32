    ...
    protected function coursecat_category_content(coursecat_helper $chelper, $coursecat, $depth) {
        ...
        if ($chelper->get_show_courses() > core_course_renderer::COURSECAT_SHOW_COURSES_COUNT) {
            ...
            if (!$chelper->get_courses_display_option('nodisplay')) {       
                ...
                //ADD BY WM 06/07/16 x nascondere corsi ad utenti non iscritti (aggiunto anche global user e db - modificato anche file lib/enrolib.php riga 598 e lib/coursecatlib.php riga 925)
                $admins = get_admins();
                $isadmin = false;
                ...
                $queryrole  = $DB->get_field('role_assignments', 'roleid', array('userid'=>$USER->id, 'contextid'=>1));
                
                if ($isadmin) {
                    $courses = $coursecat->get_courses($chelper->get_courses_display_options());
                } else {
                    if (!$queryrole) {
						if ($coursecat->id==9) {//ID 9 è la categoria My personal path
                        	...
                        	$courses  = enrol_get_my_courses('summary, summaryformat', $sortorder, '', $coursecat->id);
						} else {
                        	$courses = $coursecat->get_courses($chelper->get_courses_display_options());
                    	} 
                    } else {
